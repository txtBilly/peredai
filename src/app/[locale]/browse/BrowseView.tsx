...
export default function BrowseView({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    ...
    tickerQuery
      // Real listings lead the strip; mock/seed ones only fill in behind them.
    ...
    const timeoutId = setTimeout(
      async () => {
        ...
        let query = supabase
          ...
          .in('status', ['active', 'negotiating'])
          // Real listings always outrank mock/seed ones (is_seed=false sorts
          // first). Within each group: soonest available date first (no-date last),
          // ties break by newest-listed.
          .order('is_seed', { ascending: true })
