...
export default async function AdminReportsPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 font-display text-2xl text-paper">Очередь жалоб</h1>
      {reports.length === 0 ? (
        <p className="text-sm text-muted">Нет открытых жалоб.</p>
      ) : (
          ...
          {reports.map((r) => (
            <div key={r.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
              <div className="mb-2 flex flex-wrap items-center gap-2 text-sm">
                ...
                <span className="text-muted">пожаловался на</span>
                <span className="text-paper">{name(r.reported_user)}</span>
                {r.reported_user && (
                  <span className="text-xs text-amber-300">
                    {strikeCount.get(r.reported_user) ?? 0} предупреждений ранее
                    {profiles.get(r.reported_user)?.is_shadow_banned ? ' · теневой бан' : ''}
                  </span>
                )}
              </div>
              {r.detail && <p className="mb-3 text-sm text-muted">{r.detail}</p>}
              <div className="mb-3 text-xs text-muted">{new Date(r.created_at).toLocaleString('ru-RU')}</div>
